...
const AktivitasBerjalanPerizinan = ({ id_surat, id_surat_jenis, kategoriPerizinan, namaPerizinan, tanggalPengajuan, namaSekolah, pemohon, status, hideButton }) => {
  ...
  return (
    <CardGeneral>
      <div className="flex gap-5 w-full mx-auto" onClick={() => handleAktivitasClick(id_surat)}>
        <Link className="flex-grow" to={`/rincian-pengajuan/${id_surat}`}>
          <div>
        ...
        {hideButton ? null : (
          <div className="flex items-end">
            <a className="flex items-center w-10 h-10 bg-brand-500 rounded" href={`https://urbanscholaria.my.id/api/surat/` + id_surat + `/cetak-kwitansi`} target="_blank">
