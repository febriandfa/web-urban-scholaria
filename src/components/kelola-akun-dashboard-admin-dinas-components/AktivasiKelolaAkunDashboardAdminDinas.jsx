...
const AktivasiKelolaAkunDashboardAdminDinas = () => {
  ...
  const getDataPengguna = async () => {
    try {
      ...
      setPengguna(response?.data?.data?.filter((item) => item?.is_active === "N"));
      setLoading(false);
    } catch (error) {
      console.error(error);
      setLoading(false);
    ...
    <div>
      ...
      <div className="flex items-center gap-10 mb-8">
        <div className="w-full">
          <h1 className="text-brand-500 font-semibold text-4xl">Perlu Aktivasi</h1>
...
export default AktivasiKelolaAkunDashboardAdminDinas;
