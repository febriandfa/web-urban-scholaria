...
const ObrolanAdministrator = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await userService.postSendMessage(idRoomChat, { receiver_user_id: receiver, message: newMessage.text });
      ...
      <div className="grid grid-cols-7 gap-x-10">
        <div className="col-span-2">
          {roomList.map((room, index) => (
            <div className="p-2 mb-2 bg-neutral-100 w-full flex items-center gap-3 border border-neutral-300 rounded-lg" key={index} onClick={chatListOnClick(room?.id)}>
