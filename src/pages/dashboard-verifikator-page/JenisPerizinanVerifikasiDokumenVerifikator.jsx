...
import CariPerizinanDashboardAdministrator from "../../components/dashboard-administrator-components/CariPerizinanDashboardAdministrator";
import LoadingPopup from "../../components/popup-components/LoadingPopup";

const JenisPerizinanVerifikasiDokumenVerifikator = () => {
  ...
  useEffect(() => {
    jenisPerizinan.forEach(async (jenis) => {
      try {
        const response = await userService.getSuratStatusVerifVerifikator(jenis.id);
    ...
    <MainPageLayout>
      ...
      {jenisPerizinan.map((jenis, index) => (
        <div className="relative" key={index}>
          ...
          <Link className="flex justify-between items-center bg-neutral-100 rounded-lg py-4 px-8 my-5 shadow-md" onClick={() => handleJenisOnClick(jenis.id)} to="/verifikasi-dokumen-verifikator">
...
export default JenisPerizinanVerifikasiDokumenVerifikator;
