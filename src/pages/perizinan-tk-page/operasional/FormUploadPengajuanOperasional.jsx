  ...
  return (
    <div>
      ...
      <form className="w-4/5 mx-auto" action="#" method="post">
        ...
        {!showKetentuanSection ? (
          <button type="button" onClick={toggleKetentuanSection} className={`py-2 px-4 text-center cursor-pointer bg-brand-200 w-full rounded-lg text-base font-semibold text-neutral-800 block`}>
            Selanjutnya
          </button>
        ) : (
          <button type="button" onClick={() => handleSubmitPengajuan()} className={`py-2 px-4 text-center cursor-pointer bg-brand-500 w-full rounded-lg text-base font-semibold text-white block`}>
            Ajukan Perizinan
          </button>
        )}
      </form>
